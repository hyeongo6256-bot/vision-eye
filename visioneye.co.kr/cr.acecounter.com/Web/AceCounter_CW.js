var _SGC = 'BR3A44333218526';

function _AceGScript(O, T) {
...
if (typeof(_AceGID) == 'object') {
    var _ACE_GUID = _AceGScript(_AceGID, 'CW');
    var _UD = 'undefined';
    if (typeof(_ACE_GUID) != _UD) {
        var CW_GUL = _ACE_GUID[0];
        var CW_GPT = _ACE_GUID[1];
        var CW_GCD = _ACE_GUID[2];
        ...
        function _ISCK(a, b) {
            _ASC(a, b, 2);
            var _uci = _AGC("ACEUCI");
            return (b.length > 0 ? b : '0') + '.' + ((document.cookie.indexOf(a + "=" + b) > -1) ? 1 : 0) + '.' + _uci.length;
        }
        ...
        var _BID = _AGC("_fbp");
        var _FV = _AGC("ACEFCID");
        if (String(_FV).length != 28) {
            _FV = SetUID();
            _ASC("ACEFCID", _FV, 86400 * 30 * 12);
        };

        function A_FCV(_fs, A) {
            var _dt = new Date();
            var _ts = Math.floor(_dt.getTime() / 1000) % A;
            var ucd = String(_fs).replace('UID-', '') + "-" + _ts;
            return String(ucd).substr(0, 31);
        }
        var _FCV = A_FCV(_FV, 86400);
        window._AcePID = escape(CW_GUL + ":" + CW_GPT + '/?cuid=' + CW_GCD + '&md=p&FCV=' + _FCV + '&dm=' + location.hostname);
        var _AIO = _NIM();
        var _AIU = _NIM();
        var _AIW = _NIM();
        var _AIX = _NIM();
        var _AIB = _NIM();
        var __hdki_xit = _NIM();
        var _gX = '/?xuid=' + CW_GCD + '&sv=' + _JV,
            _gF = '/?fuid=' + CW_GCD + '&sv=' + _JV,
            _gU = '/?uid=' + CW_GCD + '&sv=' + _JV + "&FCV=" + _FCV,
            _gE = '/?euid=' + CW_GCD + '&sv=' + _JV,
            _gW = '/?wuid=' + CW_GCD + '&sv=' + _JV,
            _gO = '/?ouid=' + CW_GCD + '&sv=' + _JV,
            _gB = '/?buid=' + CW_GCD + '&sv=' + _JV;
        ...
        if (_APY == "1") {
            var _msp = "member_key=" + _id + "&jid=" + _jid;
        } else {
            var _msp = "mset=" + _mset;
        }
        ...
        function ACEF_Tracking(a, b, c, d, e, f) {
            ...
            if (_IDV(CW_GCD)) {
                ...
                var _AS_rl = CU_rl + '/?xuid=' + CW_GCD + '&url=' + escape(_AF_rl) + '&xlnk=' + escape(f) + '&fdv=' + b + '&xidx=' + e + '&';
        ...
        function _ER(a, b, c, d) {
            ...
            if (String(navigator.userAgent).indexOf('Mobile Safari') != -1) {
                return 0;
            };
        ...
        if (top && typeof top == 'object' && _ILF(top.frames)) {
            ...
            if (_rl != document.URL) _afrm = true;
        };
        if (_afrm) {
            ...
            if (_rf == _prl) {
                ...
                if (_IX(document.cookie, 'ACENASP_CK=' + escape(_rf)) > -1) {
                    _rf = _prl;
                } else {
                    if (_IX(_prl, '?') > 0) {
        ...
        if (_AKC != '') {
            var _R_a = _AKC.split(',');
            var _ak2 = '';
            var _ak_list = _ak.split('&');
            for (var i = 0; i < _ak_list.length; i++) {
                var cnt = 0;
                for (var a = 0; a < _R_a.length; a++) {
                    if (_ak_list[i].indexOf(_R_a[a]) == 0) cnt++;
                };
                if (cnt > 0) continue;
                if (_ak2 != '') _ak2 += '&';
                _ak2 += _ak_list[i];
        ...
        } else if ((_st = _IX(_ua, "chrome")) > 0) {
            _cmp = "Chrome";
            _vs = parseFloat(_ua.substring(_st + 7, _ua.indexOf(' ', _st + 7))).toString();
        } else if ((_st = _IX(_ua, "safari")) > 0) {
            _cmp = "Safari";
            _vs = parseFloat(_ua.substring(_st + 7, _ua.indexOf(' ', _st + 7))).toString();
        ...
        function LA_CW_Click() {
            ...
            var s = {
                Lp: 'a.tagName=="IMG"||a.tagName=="B" || a.tagName=="I" || a.tagName== "U" || a.tagName== "FONT" || a.tagName=="STRONG" || a.tagName=="I" || a.tagName=="A" || a.tagName=="AREA" || a.tagName=="SPAN"',
                tt: 'title',
                l: 'load',
                c: 'click',
                f: 'function',
            ...
            if (p.u().charAt(1) != ':') {
                switch (arguments.length) {
                    ...
                    case 2:
                        if (ssn.n != ssn.run) {
                            if ((f.e(f.d(arguments[0]), '.SWF') > -1) && (f.e(arguments[0], '/') != 0)) {
                                if (f.a(arguments[1]) != '') {
        ...
        if (_rl.indexOf(';') > 0) {
            var _spt = _rl.indexOf(';');
            _rl = _rl.substr(0, _spt);
        };
